...
/**
 * ViewResolver接口的简单实现，可以直接根据对应URL的视图名解析，不需要显式进行映射定义。
 * 字符串视图名直接匹配视图资源名称（例如，字符串名称就是资源文件名），不需要为每个视图再另外专门定义映射。
 ...
 *
 * 支持AbstractUrlBasedView的子类，像InternalResourceView和FreeMarkerView。
 * 所有由当前解析器生成的视图的视图类型可以通过viewClass指定。
 ...
 *
 * 视图名可以是资源URL本身，或者可以通过指定前缀/后缀进行参数化。
 * 支持显式将RequestContext属性提供给所有的视图。
 ...
 *
 * 重定向URL比较特别，需要通过redirect:前缀进行指定。例如，edirect:myAction会重定向到一个URL，而不是被解析成一个标准的视图名称。
 * 通常用于在完成表单工作流程后，重定向到一个控制器URL。
 ...
 *
 * 此外，转发URL可以通过forward:进行指定。例如，forward:myAction会转发到指定的URL，而不是解析成一个标准视图名称。
 * 通常用于控制器URL，而不是JSP URL——使用逻辑视图名。
 ...
 *
 * 注意：链接视图解析器时，UrlBasedViewResolver会检查（AbstractUrlBasedView#checkResource）指定的资源实际是否存在，
 * 而InternalResourceView通常不会去这么做。这样，UrlBasedViewResolver对给定视图名总是会返回一个视图。
 * 因此，它应该配置作为视图解析链的最后一个元素。
